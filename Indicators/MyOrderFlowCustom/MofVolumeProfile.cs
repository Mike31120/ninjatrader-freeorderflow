...
using InvestSoft.NinjaScript.VolumeProfile; // <-- on utilise les classes utilitaires d'InvestSoft
#endregion
    ...
    public class MofVolumeProfile : Indicator
    {
        ...
        protected override void OnStateChange()
        {
            ...
            else if (State == State.Configure)
            {
                ...
                AddDataSeries(
                    (ResolutionMode == MofVolumeProfileResolution.Tick) ? BarsPeriodType.Tick : BarsPeriodType.Minute,
                    Resolution
                );

                Profiles = new List<MofVolumeProfileData>
                {
                    new MofVolumeProfileData { StartBar = 0, EndBar = 0 }
                };
            }
        ...
        private void DetectLevels(MofVolumeProfileData prof)
        {
            ...
            {
                int start = i, end = i;
                while (end + 1 < prices.Count && Math.Abs(smooth[end + 1] - smooth[end]) < EPS) end++;
                ...
                if (higher && v >= minVol)
                {
                    for (int j = start; j <= end; j++) prof.HvnZones.Add(prices[j]);
                    int idx = GetPlateauIndex(start, end, vols, true, PlateauSelectionMode.Central);
                    int extremeIdx = idx; double extremeVol = vols[idx];
                    for (int j = 1; j <= n; j++)
                    {
                        int left = idx - j;
                        if (left >= 0 && vols[left] > extremeVol) { extremeVol = vols[left]; extremeIdx = left; }
                        int right = idx + j;
                        if (right < vols.Count && vols[right] > extremeVol) { extremeVol = vols[right]; extremeIdx = right; }
                    }
                ...
                if (lower)
                {
                    for (int j = start; j <= end; j++) prof.LvnZones.Add(prices[j]);
        ...
        private int GetPlateauIndex(int start, int end, List<double> vols, bool chooseMax, PlateauSelectionMode mode)
        {
            ...
            for (int i = start + 1; i <= end; i++)
            {
                ...
                {
                    extreme = vv; indices.Clear(); indices.Add(i);
                }
                else if (vv == extreme)
                {
                    indices.Add(i);
            ...
            switch (mode)
            {
                case PlateauSelectionMode.Highest: return indices[indices.Count - 1];
                case PlateauSelectionMode.Central: return indices[indices.Count / 2];
                default: return indices[0];
            }
        }
        #endregion
        ...
        protected override void OnRender(ChartControl chartControl, ChartScale chartScale)
        {
            ...
            foreach (var profile in Profiles)
            {
                ...
                // NEW: nombres sur chaque barre (volume total par prix)
        ...
        public override void OnRenderTargetChanged()
        {
            ...
            if (RenderTarget != null)
            {
                ...
                backgroundBrushDX = ProfileBackgroundBrush != null ? ProfileBackgroundBrush.ToDxBrush(RenderTarget) : null;
        ...
        #region Properties
        // Setup
        [Display(Name = "Display mode", Order = 1, GroupName = "Setup")]
        public MofVolumeProfileMode DisplayMode { get; set; }

        [NinjaScriptProperty, Display(Name = "Profile Period", Order = 1, GroupName = "Setup")]
        public MofVolumeProfilePeriod Period { get; set; }

        [NinjaScriptProperty, Display(Name = "Resolution Mode", Order = 2, GroupName = "Setup")]
        public MofVolumeProfileResolution ResolutionMode { get; set; }

        [NinjaScriptProperty, Display(Name = "Resolution", Order = 3, GroupName = "Setup")]
        public int Resolution { get; set; }

        [Range(10, 90), Display(Name = "Value Area (%)", Order = 7, GroupName = "Setup")]
        ...
        [Display(Name = "Max profile width (px)", Order = 2, GroupName = "Visual")]
        public int MaxWidthPixels { get; set; }

        [Range(1, 100), Display(Name = "Profile opacity (%)", Order = 3, GroupName = "Visual")]
        public int Opacity { get; set; }

        [Range(1, 100), Display(Name = "Value area opacity (%)", Order = 4, GroupName = "Visual")]
        public int ValueAreaOpacity { get; set; }

        [Display(Name = "Show POC", Order = 5, GroupName = "Setup")]
        public bool ShowPoc { get; set; }

        [Display(Name = "Show Value Area", Order = 6, GroupName = "Setup")]
        public bool ShowValueArea { get; set; }

        [XmlIgnore, Display(Name = "Color for profile", Order = 10, GroupName = "Visual")]
        public Brush VolumeBrush { get; set; }
        [Browsable(false)] public string VolumeBrushSerialize { get => Serialize.BrushToString(VolumeBrush); set => VolumeBrush = Serialize.StringToBrush(value); }

        [XmlIgnore, Display(Name = "Color for buy", Order = 11, GroupName = "Visual")]
        public Brush BuyBrush { get; set; }
        [Browsable(false)] public string BuyBrushSerialize { get => Serialize.BrushToString(BuyBrush); set => BuyBrush = Serialize.StringToBrush(value); }

        [XmlIgnore, Display(Name = "Color for sell", Order = 12, GroupName = "Visual")]
        public Brush SellBrush { get; set; }
        [Browsable(false)] public string SellBrushSerialize { get => Serialize.BrushToString(SellBrush); set => SellBrush = Serialize.StringToBrush(value); }

        [XmlIgnore, Display(Name = "Color for outline", Order = 13, GroupName = "Visual")]
        public Brush OutlineBrush { get; set; }
        [Browsable(false)] public string OutlineBrushSerialize { get => Serialize.BrushToString(OutlineBrush); set => OutlineBrush = Serialize.StringToBrush(value); }

        [XmlIgnore, Display(Name = "Profile background", Order = 14, GroupName = "Visual")]
        public Brush ProfileBackgroundBrush { get; set; }
        [Browsable(false)] public string ProfileBackgroundBrushSerialize { get => Serialize.BrushToString(ProfileBackgroundBrush); set => ProfileBackgroundBrush = Serialize.StringToBrush(value); }

        [XmlIgnore, Display(Name = "HVN Highlight", Order = 15, GroupName = "Visual")]
        public Brush HvnHighlightBrush { get; set; }
        [Browsable(false)] public string HvnHighlightBrushSerialize { get => Serialize.BrushToString(HvnHighlightBrush); set => HvnHighlightBrush = Serialize.StringToBrush(value); }

        [XmlIgnore, Display(Name = "LVN Highlight", Order = 16, GroupName = "Visual")]
        public Brush LvnHighlightBrush { get; set; }
        [Browsable(false)] public string LvnHighlightBrushSerialize { get => Serialize.BrushToString(LvnHighlightBrush); set => LvnHighlightBrush = Serialize.StringToBrush(value); }

        [XmlIgnore, Display(Name = "POC Highlight", Order = 17, GroupName = "Visual")]
        public Brush PocHighlightBrush { get; set; }
        [Browsable(false)] public string PocHighlightBrushSerialize { get => Serialize.BrushToString(PocHighlightBrush); set => PocHighlightBrush = Serialize.StringToBrush(value); }

        // NEW: Texte sur barres
        [Display(Name = "Show Bar Volume Text", Order = 18, GroupName = "Visual")]
        public bool ShowBarVolumeText { get; set; }

        [Range(1, 100), Display(Name = "Bar Text Opacity (%)", Order = 19, GroupName = "Visual")]
        ...
        [XmlIgnore, Display(Name = "Bar Text Color", Order = 21, GroupName = "Visual")]
        ...
        [Range(1, 20), Display(Name = "Smoothing Window", Order = 1, GroupName = "Levels")] public int SmoothingWindow { get; set; }
        [Range(1, 20), Display(Name = "Neighbor Bars", Order = 2, GroupName = "Levels")] public int NeighborBars { get; set; }
        [Range(0, 100), Display(Name = "Min Vol % of POC", Order = 3, GroupName = "Levels")] public int MinVolumePctOfPoc { get; set; }
        [Display(Name = "Min Distance (ticks)", Order = 5, GroupName = "Levels")] public int MinDistanceTicks { get; set; }
        [Display(Name = "Max Levels", Order = 6, GroupName = "Levels")] public int MaxLevels { get; set; }
        [Display(Name = "Use Global Levels", Order = 7, GroupName = "Levels")] public bool UseGlobalLevels { get; set; }
        #endregion
    }
}
...
namespace NinjaTrader.NinjaScript.Indicators
{
    public partial class Indicator : NinjaTrader.Gui.NinjaScript.IndicatorRenderBase
    {
        ...
        public MyOrderFlowCustom.MofVolumeProfile MofVolumeProfile(MofVolumeProfilePeriod period, MofVolumeProfileResolution resolutionMode, int resolution)
            => MofVolumeProfile(Input, period, resolutionMode, resolution);
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
    public partial class MarketAnalyzerColumn : MarketAnalyzerColumnBase
    {
        public Indicators.MyOrderFlowCustom.MofVolumeProfile MofVolumeProfile(MofVolumeProfilePeriod period, MofVolumeProfileResolution resolutionMode, int resolution)
            => indicator.MofVolumeProfile(Input, period, resolutionMode, resolution);
